...
object structural_type {
	...
	*/
	...
	val observer: Int => Unit = (state: Int) => println("got one! " + state)
                                                  //> observer  : Int => Unit = <function1>
	
	val subject = new SubjectFunc {
		type State = Int
	}                                         //> subject  : proscala.chap14.structural_type.SubjectFunc{type State = Int} = 
                                                  //| proscala.chap14.structural_type$$anonfun$main$1$$anon$1@511baa65
	subject.addObserver(observer)
	subject.notifyObservers(10)               //> got one! 10
}