...
object func {
  ...
  // ex) http://www.scala-lang.org/api/current/scala/Function1.html
 	...
 	// Function1[-Papa, +Papa]
	var test: Papa => Papa = (c: Papa) => new Papa
                                                  //> test  : proscala.chap10.func.Papa => proscala.chap10.func.Papa = <function1>
                                                  //| 
  // 아래 세개의 수정함수는 역변, 가변성을 만족한다.
	test = (c: GrandPapa) => new Son
	
	test = (c: GrandPapa) => new Papa
	test = (c: Papa) => new Son
