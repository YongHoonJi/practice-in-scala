...
object selftype_cake_pattern {
	...
	object MyApp extends App with Database with BizLogic with WebUI;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(912); 
	
	MyApp.run
  // 상속기반 구현 : 아래와 같이 자기 참조 타입 대신 상속을 사용할 수 있다.
	object Myapp2 extends AppInheritance with Database with BizLogic with WebUI;$skip(139); 
	Myapp2.run()}
}
